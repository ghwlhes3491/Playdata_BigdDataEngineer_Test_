...
//      super.memberId = memberId;
   ...
   @Override
   public String toString() {	//문자열 변환하도록 한것
	   //부모로부터의 물려받은 멤버는 private이면 값을 그대로 가져올 수 없음
      return "[일반자식] "+ getMemberId() + ", " + getMemberPw() + ", " + getName() + ", " + getMobile() 
      			+ ", " + getEmail() + ", " + getEntryDate() + ", " + getGrade()
				+ ", " +  getManager()  + ", " + mileage;

	//이게 무슨 의미? 뒤에 뭔가 나오게 하는 건가?
   }
